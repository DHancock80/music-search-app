...
if search_type == 'Album':
    suggestions = album_list
elif search_type == 'Artist':
    suggestions = artist_list
else:
    suggestions = song_list

search_query = st.text_input('Enter your search:', '', autocomplete='on', placeholder='Type to search...')

# Autocomplete dropdown using st.selectbox
if len(search_query) >= 2:
    matches = process.extract(search_query, suggestions, limit=5)
    suggestion_options = [match[0] for match in matches]
    if suggestion_options:
        selected = st.selectbox('Did you mean:', options=[''] + suggestion_options)
        if selected:
...
counts = {
    'All': len(results),
    'Album': len(results[results['Format'].str.lower().isin(['album', 'compilation', 'comp'])]) if 'Format' in results.columns else 0,
...
format_filter = st.radio(
    'Filter by format:',
    [f"{opt} ({counts[opt]})" for opt in format_options],
    horizontal=True
)

# Extract the filter choice
selected_format = format_filter.split(' ')[0]

if selected_format != 'All':
    if 'Format' in results.columns:
        if selected_format == 'Album':
            results = results[results['Format'].str.lower().isin(['album', 'compilation', 'comp'])]
        else:
            results = results[results['Format'].str.lower() == selected_format.lower()]
